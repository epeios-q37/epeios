...
async def updateBoard(dom, board):
  """Updates the game board using the board dictionary."""

  await dom.setValues(board)
  ...
  if winner in ['1', '2']:
    ...
    await notify(dom, "Player {}, click one of your pit.".format(playerTurn))
...
async def acSubmit(dom, id):
  ...
  if ( playerTurn == '1' and response not in PLAYER_1_PITS )\
    ...
    await notify(dom, 'Please pick a pit on your side of the board.')
    return
  if gameBoard.get(response) == 0:
    await notify(dom, 'Please pick a non-empty pit.')
    return
